...
#define MACRO_POINT_CLOUD_LOCAL_TOPIC "/point_cloud_map/local_map"
#define MACRO_POINT_CLOUD_GLOBAL_TOPIC "/point_cloud_map/global_map"
// 坐标系名称
// TODO 3.3.3
#define MACRO_CAMERA_OPTICAL_NAME "camera_depth_optical_frame"//深度相机光心坐标系
#define MACRO_MAP_LINK_NAME "map"//地图坐标系
...
public:
        ...
        : ExperNodeBase(nArgc, ppcArgv, pcNodeName)
    {
        ...
        mPubLocalPCMap  =mupNodeHandle->advertise<sensor_msgs::PointCloud2>(MACRO_POINT_CLOUD_LOCAL_TOPIC,1);
        mPubGlobalPCMap = mupNodeHandle->advertise<sensor_msgs::PointCloud2>(MACRO_POINT_CLOUD_GLOBAL_TOPIC,1);
        ...
        ROS_INFO("Initialized, mfPCFilterThres = %.4f", mfPCFilterThres);
        // 初始化位姿状态
...
public:
    ...
    void CreateLocalPointCloudMap(void)
    {
        // 清除局部点云
        mLocalPCMap.clear();
    //    ROS_INFO_STREAM("into:CreateLocalPointCloudMap");
        ...
        for (size_t nIdY = 0; nIdY < mnRows; ++nIdY)
        {
            for (size_t nIdX = 0; nIdX < mnCols; ++nIdX)
            {
                ...
                pt.y=y;
                pt.z=z;
                // ROS_INFO_STREAM("nldY:"<<nIdY<<"   nldX:"<<nIdX<<"add----pt.b:"<<pt.b<<"     pt.g:"<<pt.g<<"   pt.r:"<<pt.r<<" ");
                mLocalPCMap.push_back(pt);///！push_back把点加入局部点云地图中//动态增长：增加新元素，如果超过当时的容量，则容量会扩充至原来的两倍。

            }

        }
         cout<<"LocalPointCloudMap Set Done!"<<endl;
        // TODO 3.3.2 转换成为 ROS 的消息格式
        pcl::toROSMsg(mLocalPCMap,mMsgLocalPCMap);
        // NOTICE 注意这里的 MACRO_CAMERA_OPTICAL_NAME 需要自己设置
        mMsgLocalPCMap.header.frame_id = MACRO_CAMERA_OPTICAL_NAME;//深度相机光心坐标系
        // TODO 3.3.2 发布
        mPubGlobalPCMap.publish(mMsgLocalPCMap);
    }
    ...
    void UpdateGlobalPointCloudMap(void)
    {
       ...
       //Step 1.1 滤波
        ...
        //pcl::PointCloud<pcl::PointXYZRGB> cloud_filtered;//滤波后结果
        pcl::StatisticalOutlierRemoval<pcl::PointXYZRGB>  statFilter;
         statFilter.setInputCloud(mLocalPCMap.makeShared());
         statFilter.setMeanK(mnPCMeanK);
         statFilter.setStddevMulThresh(mfPCFilterThres);
        ...
        for (auto &oldPt : tmpPCMapFiltered)
        {
            ...
            // 访问元素: tvAfter3d.x()
            pcl::PointXYZRGB newPt;//和oldPt相比：颜色不变，坐标通过mCurrTransform映射
            newPt.b=oldPt.b;
            newPt.g=oldPt.g;
            newPt.r=oldPt.r;
            newPt.x=tvAfter3d.x();
        ...
        // Step 3 对全局点云mGlobalPCMap进行降采样cloud_downsampled
        // TODO 3.3.4
        pcl::PointCloud<pcl::PointXYZRGB> cloud_downsampled;//降采样后结果
        pcl::VoxelGrid<pcl::PointXYZRGB> voxelSampler;
        voxelSampler.setInputCloud(mGlobalPCMap.makeShared());
        voxelSampler.setLeafSize(mfPCLeafSize,mfPCLeafSize,mfPCLeafSize);
        voxelSampler.filter(cloud_downsampled);

        // Step 4 发布全局点云地图
        mGlobalPCMap=cloud_downsampled;
        pcl::toROSMsg(mGlobalPCMap, mMsgGlobalPCMap);
        mMsgGlobalPCMap.header.frame_id = MACRO_MAP_LINK_NAME;
        // 发布
...
private:
    ...
    std::unique_ptr<image_transport::ImageTransport> mupImgTrans; ///< Image Transporter
    ...
    pcl::PointCloud<pcl::PointXYZRGB> mLocalPCMap;  ///< 彩色局部点云对象
    pcl::PointCloud<pcl::PointXYZRGB> mGlobalPCMap; ///< 彩色全局点云对象
    ...
    cv::Mat mimgScaledLastRGB;   ///< 上次获得的缩放后PointXYZRGB的彩色图像
