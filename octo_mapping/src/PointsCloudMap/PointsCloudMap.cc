...
#define MACRO_LOCAL_MAP                         "/point_cloud_map/local_map"
#define MACRO_GLOBAL_MAP                         "/point_cloud_map/global_map"
...
public:
        ...
        : ExperNodeBase(nArgc, ppcArgv, pcNodeName)
    {
        ...
        mPubLocalPCMap  = mupNodeHandle->advertise<sensor_msgs::PointCloud2>(MACRO_LOCAL_MAP,1);
        mPubGlobalPCMap = mupNodeHandle->advertise<sensor_msgs::PointCloud2>(MACRO_GLOBAL_MAP,1);
    ...
    void CreateLocalPointCloudMap(void)
    {
        ...
        for(size_t nIdY = 0; nIdY < mnRows; ++nIdY)
        {
            for(size_t nIdX = 0; nIdX < mnCols; ++nIdX)
            {
                ...
                float x = (1/mDepthCameraIntrinsic.fx)*(u*z-z*mDepthCameraIntrinsic.cx);
                float y = (1/mDepthCameraIntrinsic.fy)*(v*z-z*mDepthCameraIntrinsic.cy);

                // b: color[0] g: color[1] r: color[2]
                cv::Vec3b color = mimgScaledLastRGB.at<cv::Vec3b>(nIdX, nIdY);
