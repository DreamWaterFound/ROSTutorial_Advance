...
//
#define MACRO_SRC_COLORED_DEPTH_RES_TOPIC       "/image_converter/colored_depth"
#define MACRO_SRC_RGB_GREY_RES_TOPIC            "/image_converter/RGB_grey"
...
public:
        ...
        : ExperNodeBase(nArgc, ppcArgv, pcNodeName)
    {
        ...
        // 彩色灰度化图像发布器
        mImgPubRGBGrey = mupImgTrans->advertise(MACRO_SRC_RGB_GREY_RES_TOPIC , 1);
        // 伪彩色化深度图像发布器
...
public:
    ...
    void ImageRGBCallback(const sensor_msgs::ImageConstPtr& pMsg)
    {
        ...
        cv::Mat RGB_GreyImg;
        cv::cvtColor(mimgScaledLastRGB, RGB_GreyImg, CV_RGB2GRAY);

        // 消息类型转换
        sensor_msgs::ImagePtr processed_RGB_img = cv_bridge::CvImage(std_msgs::Header(), "mono8", RGB_GreyImg).toImageMsg();
        // 发布消息
        mImgPubRGBGrey.publish(processed_RGB_img);

        //OpenCV显示图像
        if(!mimgScaledLastRGB.empty())
        {
            cv::imshow("Input", mimgScaledLastRGB);
        }

        if(!RGB_GreyImg.empty())
        {
    ...
    void ImageDepthCallback(const sensor_msgs::ImageConstPtr& pMsg)
    {
        ...
        // 根据需要自由补充代码
        // 调用如下函数实现深度图向彩色图的转换
        cv::Mat Colored_Depth_Img;
        ColoredDepth(mimgScaledLastDepth, Colored_Depth_Img);

        // TODO 3.2.2 发布图像
        // 消息类型转换

        sensor_msgs::ImagePtr processed_Depth_img = cv_bridge::CvImage(std_msgs::Header(), "bgr8", Colored_Depth_Img).toImageMsg();
        // 发布消息
        mImgPubColoredDepth.publish(processed_Depth_img);
...
private:
    ...
    // TODO 3.2.2 添加发布器
    image_transport::Publisher                       mImgPubRGBGrey;            //彩色灰度化图像发布器
    image_transport::Publisher                       mImgPubColoredDepth;       //伪彩色化深度图像发布器
