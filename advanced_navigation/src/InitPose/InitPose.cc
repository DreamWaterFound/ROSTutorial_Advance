...
public:
        ...
        : ExperNodeBase(nArgc, ppcArgv, pcNodeName)
    {
        ...
        //mClientGzbPose = mupNodeHandle->serviceClient<gazebo_msgs::ModelStates>(MACRO_GAZEBO_MODEL_STATE_SRV);
    ...
    void Run(void) override
    {
        ...
        // Step 1 从 Gazebo 获取机器人初始位置
        {
            ...
            {
                ros::Duration(0.1).sleep();
                ROS_INFO("Waiting for Gazebo service");
            }
            ...
            ROS_INFO("Get init pose SUCCESSED!");
...
private:
    ...
    void SetInitPoseMsg(void)
    {
        ...
        //2.2.1的yaw角初始值怎么得到的？应该为Gazebo中实际的角度0度，否则costmap和实际的全局地图会冲突
...
private:
    ...
    // TODO 2.2.3
    //gazebo_msgs::ModelStates                  mSrvGzbModelState;        ///< 得到的 Gzb 中机器人状态的服务类型
